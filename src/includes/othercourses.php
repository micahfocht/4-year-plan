<?php
$query = oci_parse($database, "select unique e.e_semester from ENROLL e, STUDENT s where e.e_stid = s.s_id and s.s_email = :email");
oci_bind_by_name($query,":email",$email);
oci_execute($query);
while($code = oci_fetch_array($query, OCI_ASSOC+OCI_RETURN_NULLS)) {
    $season = substr($code["E_SEMESTER"], -2);
    echo($code["E_SEMESTER"]);
    echo("<br>");
    if ($season != "SP" && $season != "FA") {
        ...
        oci_bind_by_name($query, ":email", $email);
        oci_bind_by_name($query, ":semester", $code['E_SEMESTER']);
        oci_execute($query);
        while ($code = oci_fetch_array($query, OCI_ASSOC + OCI_RETURN_NULLS)) {
            echo('
                                        <!-- COL 2 ROW -->
