...
<html>
    ...
    <body>
        ...
        while ($row = oci_fetch_array($query, OCI_ASSOC)) {
            foreach ($row as $item){
                echo('<div class="col-sm-5 major"><h3>Major: ' . $item . "</h3></div>");
            }
        }
        $query = oci_parse($database, "select s_name from STUDENT s where s.s_email = :email");
        oci_bind_by_name($query,":email", $email); //Get the name
        oci_execute($query);
        while ($row = oci_fetch_array($query, OCI_ASSOC)) {
            foreach ($row as $item){
                echo('<div class="col-sm-5 name"><h3><a href="logout.php">' . $item . "</a></h3></div>");
            }
        }
        ?>
        <div class="col-sm-1"></div>
